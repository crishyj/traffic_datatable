		...
		<script class="init">
			...
			$(document).ready(function () {		
				...
				editor.on( 'preSubmit', function (e, data, action) {		
					console.log(data);
					if(action != 'edit'){
						data.data[0].creator = "<?php echo $_SESSION['log_name']; ?>";
						data.data[0].created = "<?php echo date('Y-m-d H:i:s'); ?>";	
					}								
				});
				...
				$("#example > tbody > tr").each(function(index, tr) {
					...
					$(`#${id} td`).each(function(i, item) {
						console.log($(`#${id} td:nth-child(11)`).val());						
					});
				});						
			});
					...
					$("#example > tbody > tr").each(function(index, tr) {
						...
						$(`#${id} td`).each(function(i, item) {
							...
							if ($(item).text() .indexOf(vehicle_license) >= 0) {
								let driver_name = $(`#${id} td:nth-child(3)`).text();
								let company_name = $(`#${id} td:nth-child(4)`).text();
								let destination = $(`#${id} td:nth-child(6)`).text();
                        ...
                        $(o).autocomplete(
                            {
                                ...
                                {
                                    $("#DTE_Field_driver_name").val(ui.item.meta[2]);
                                    $("#DTE_Field_company_name").val(ui.item.meta[3]);
                                    $("#DTE_Field_destination_and_purpose").val(ui.item.meta[5]);
